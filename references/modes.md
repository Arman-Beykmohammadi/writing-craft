...
2. **Scope.** Critique and Detect change nothing. An ordinary cleanup request ("clean up", "tighten", "polish", "kürzen") allows minimal, targeted wording edits that keep the structure and argument, and it authorizes Tier 1B clarity edits ("in order to" → "to") and cutting single empty connectors ("Moreover,") in the requested scope even where those entries are `relaxed`; report them as clarity edits, not as AI tells. Rebuilding, reordering, or condensing needs a request broad enough to permit it. For a large file with a clearly named section, work on that section. When the scope is genuinely unclear, use the narrowest relevant scope or ask one question.
...
- **Embedded use:** when another task uses this skill for a pull request description, commit message, or document it is writing, return only the final text, no report. The fact rules still apply in full: removing a tell never turns a vague claim into a specific one ("ensuring seamless performance" does not become "improved performance").
